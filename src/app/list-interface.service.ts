...
export class ListInterfaceService {
...
export class Spcrud {
  ...
  refreshDigest(): Promise<any> {
    const svc = this;
    const svcHeaders = this.http.post(this.baseUrl + '/_api/contextinfo', null, this.options).toPromise().then(function (res: Response) {
      this.svcHeaders = res.json();
    });
    return this.http.post(this.baseUrl + '/_api/contextinfo', null, this.options).toPromise().then(function (res: Response) {
      ...
      svc.headers.append('X-RequestDigest', this.svcHeaders.GetContextWebInformation.FormDigestValue);
    });
  }
  ...
  jsonRead(listName: string): Promise<any> {
    ...
    return this.getCurrentUser().then(function (res: any) {
      ...
      return svc.http.get(url, svc.options).toPromise().then(function (res2: Response) {
        ...
        // Parse JSON response
        const d2 = res2.json().then(res3 =>{
          this.d2 = res3.d;          
        });
        //const d2 = res2.json().d;
        if (this.d2.results.length) {
          return this.d2.results[0];
        } else {
          return null;
        }
