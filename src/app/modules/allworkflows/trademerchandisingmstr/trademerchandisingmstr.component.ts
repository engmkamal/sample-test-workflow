...
import { TmliviewbtncstmComponent } from '../../trademerchandising/tmliviewbtncstm/tmliviewbtncstm.component';
...
@Component({
  selector: 'app-trademerchandisingmstr',
  templateUrl: './trademerchandisingmstr.component.html',
  styleUrls: ['./trademerchandisingmstr.component.scss']
})



export class TrademerchandisingmstrComponent implements OnInit { 
  ...
  ngOnInit() {     
    this.listInfo.name = "ShopSignboardRequestMaster";
    this.listInfo.query = 'RequestID'+","+'PendingTo'+","+'RequestStatus'+","+'ID'+","+'Modified'+","+'Created'+","+'PendingTo'+","+'Author/ID'+","+'Author/Title';
    this.listInfo.expand = 'Author';
     
    this.sharepointlistService.getSPLoggedInUser().then((res)=> {
      from(
        this.sharepointlistService.getUsersWFOldMasterListItems(this.listInfo, res)
        //this.sharepointlistService.getUsersTMMasterListItems(this.listInfo, res)
        ).subscribe(
          (items) =>{ 
            this.rowData = items;
          },
