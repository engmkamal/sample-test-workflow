...
@Component({
  selector: 'app-testresultinputfields',
  templateUrl: './testresultinputfields.component.html',
  styleUrls: ['./testresultinputfields.component.scss'],
  providers: [
...
export class TestresultinputfieldsComponent implements OnInit, AfterViewInit{    
  ...
  setExistingTestResults(testResultSets: ITestResult[], testParamValue): FormArray {
    ...
    else if(testParamValue.Title.Code == "RUBB"){
      testResultSets.forEach(s =>{
        formArray.push(this.formBuilder.group({
            SampleID: s.SampleID,
            Results: this.formBuilder.array([
  ...
  editTestParameterOnReported(testParamValue){
    ...
    this.TestParameter.patchValue({
      PackageCondition: this.testParameter.PackageCondition,
      PacComments: this.testParameter.PacComments,
      TestPeriodFrom: this.testParameter.TestPeriodFrom,
      TestPeriodTo: this.testParameter.TestPeriodTo
    })

     //============for displaying value in Title formGroup =========
     this.TestParameter.controls.Title.patchValue({
      Title: this.testParameter.Title.Title,
      Method: this.testParameter.Title.Method
    })
  ...
  ngOnInit() {
    ...
    if(this.testParameter.TestParameterStatus == "Reported" || this.testParameter.TestParameterStatus == "Released"){
      this.createForm();
      this.editTestParameterOnReported(this.testParameter);
